...
def add_student(request):
    ...
    if request.method == 'POST':
        if student_form.is_valid():
            ...
            date_of_joining = student_form.cleaned_data.get('date_of_joining')
...
def add_course(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            messages.error(request, "Asimilar course already added")
...
def manage_staff(request):
    ...
    paginator = Paginator(allStaff, 5)
...
def admin_view_profile(request):
    try:
        # Fetch the Admin instance for the logged-in user
        admin = get_object_or_404(Admin, admin=request.user)
    except Admin.DoesNotExist:
        messages.error(request, "Admin profile not found.")
        return redirect(reverse('home'))  # Redirect to a safe page
